  ...
  return (
    <>
      ...
      <div className="div_sm">
        <div className="flex justify-center flex-none bg-gray-900 bg-opacity-80 p-2 rounded mr-2">
          <Link
            className={`px-5 py-1 text-xl ${
              isClamView ? " text-blue-700 bg-gray-200 bg-opacity-80 rounded" : "text-gray-400"
            }`}
        ...
        <div className="flex justify-center items-center mt-2 mb-4">
          <span className="mr-1 text-sm">{`ID: #`}</span>
          <input
            type="number"
            placeholder="ID"
            className="input input-bordered w-32 mr-1 text-sm"
            value={tokenId}
            onChange={handleChange}
            onKeyDown={handleKeyDown}
          />
          <Link
            className="btn btn-primary btn-md text-sm mr-1"
            role="button"
            aria-pressed="true"
            to={`/saferoom/${view}/inspect/${tokenId}`}
          >
